...
(define (averager a b c)
  ...
  (connect c me)
  me)
(define (subtractor a b difference)
  (define (process-new-value)
    (cond ((and (has-value? a) (has-value? b))
	   (set-value! difference
			  ...
			  (get-value b))
		       me))
          ((and (has-value? difference) (has-value? b))
	   (set-value! a
		       (+ (get-value difference)
			  (get-value b))
		       me))
	  ((and (has-value? difference) (has-value? a))
	   (set-value! b
		       (+ (get-value difference)
			  (get-value a))
		       me))))
  ...
  (define (me request)
    (cond ((eq? request 'I-have-a-value) (process-new-value))
          ((eq? request 'I-lost-my-value) (process-forget-value))
	  (else (error "Unknown operation: SUBTRACTOR" request))))
  (connect a me)
  (connect b me)
...
(define (divider d1 d2 q)
  (define (process-new-value)
    (cond ((and (has-value? d2)(= (get-value d2) 0))
	   (error "Cannot divide by zero" (list d1 d2 q)))
  ...
  (define (me request)
    (cond ((eq? request 'I-have-a-value) (process-new-value))
          ((eq? request 'I-lost-my-value) (process-forget-value))
	  (else (error "Unknown operation: DIVIDER" request))))
  (connect d1 me)
  (connect d2 me)
  (connect q me)
  me)
