...
(define (greatest-common-divisor n1 n2)
  (cond ((and (eq? (type n1) 'polynomial) (eq? (type n2) 'polynomial)) (gcd-poly n1 n2))
        ((and (eq? (type n1) 'number)     (eq? (type n2) 'number))     (gcd n1 n2))
	((eq? (type n1) (type n2)) (error "Operation not defined for types -- GREATEST-COMMON-DENOMINATOR" (list n1 n2)))
        (else (error "Types of n1 and n2 are different -- GREATEST-COMMON-DENOMINATOR" (list n1 n2)))))
...
(define (pseudoremainder-terms L1 L2)
  (let ((o1 (order (first-term L1)))
	(o2 (order (first-term L2))))
       (let ((integerizing-factor (expt (coeff (first-term L2)) (+ 1 (- o1 o2)))))
...
(define (reduce-terms L1 L2)
  (let ((g (gcd-terms L1 L2)))
    (list (quotient-terms L1 g) (quotient-terms L2 g))))

(define (quotient-poly p1 p2)
  (make-poly (variable p1)
             (quotient-terms (div-poly p1 p2))))

(define (reduce-poly p1 p2)
  (let ((g (gcd-poly p1 p2)))
    (list 
      (make-polynomial (variable p1) (quotient-poly p1 g))
      (make-polynomial (variable p2) (quotient-poly p2 g)))))

(define (reduce a1 a2)
  (cond
        ((and (eq? (type a1) 'polynomial) (eq? (type a2) 'polynomial)) (reduce-poly a1 a2))
        ((and (eq? (type a1) 'number) (eq? (type a2) 'number)) (reduce-terms a1 a2))
	((eq? (type a1) (type a2)) (error "Operation not defined for types -- REDUCE" (list a1 a2)))
        (else (error "Types of a1 and a2 are different -- REDUCE" (list a1 a2)))))
